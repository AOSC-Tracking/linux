...
#define LSCANFD_TXTNF(priv) (FIELD_GET(REG_TX_STATUS_BRP, lscanfd_read32(priv, LSCANFD_TX_STATUS)) != 0xff)
...
static bool lscanfd_is_txt_buf_writable(struct lscanfd_priv *priv, u8 buf)
{
	...
	if (buf_status) return false;

	tx_brp = lscanfd_read32(priv, LSCANFD_TX_STATUS);
	if (((tx_brp >> buf) & 0x1) == 0x1) return false;

	return true;
}

static bool lscanfd_insert_frame(struct lscanfd_priv *priv, const struct canfd_frame *cf, u8 buf,
				bool isfdf)
{
	u32 buf_base;
	u32 meta0 = 0;
...
static netdev_tx_t lscanfd_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	for (i=0; i<priv->ntxbufs; i++) {
		if ((((tx_stat_brp>>i) & 0x1) == 0) && (((tx_stat_bs>>(i*2)) & 0x3) == 0)){
			txtb_id = i;
			break;
		}
	}
