...
static void rtw_ethtool_get_drvinfo(struct net_device *dev, struct ethtool_drvinfo *info)
{
	...
	if (wdev) {
		sized_strscpy(info->driver, wiphy_dev(wdev->wiphy)->driver->name,
			sizeof(info->driver));
	} else {
		sized_strscpy(info->driver, "N/A", sizeof(info->driver));
	}

	sized_strscpy(info->version, DRIVERVERSION, sizeof(info->version));
	...
	if (hal_data) {
		...
		sized_strscpy(info->fw_version, "N/A", sizeof(info->fw_version));
	}

	sized_strscpy(info->bus_info, dev_name(wiphy_dev(wdev->wiphy)),
